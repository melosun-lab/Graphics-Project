...
void RasterizerQuad::rasterize(glm::mat4 M, glm::mat4 V, glm::mat4 P, glm::mat4 D, PointCloud& obj) {
	 ...
	 */
    clearBuffers();
    std::vector<glm::vec3> points = obj.get_points();
    std::vector<glm::vec3> points_norm = obj.get_points_norm();
    glm::vec4 p;
 
    for(unsigned int i = 0; i < points.size(); i++){
        p = D * P * V * M * vec4(points[i], 1.0f);
        glm::vec3 color = normalize(points_norm[i]);
        
        int x = p.x/p.w;
        int y = p.y/p.w;
        float z = p.z/p.w;
        float r = 0.5f * color.x + 0.5f;
        float g = 0.5f * color.y + 0.5f;
        float b = 0.5f * color.z + 0.5f;
...
void RasterizerQuad::updatePointSize(bool up){
    if(up){
        size += 1;
    }
    else{
